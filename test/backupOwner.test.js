...
contract("Inherichain (Backup Owner Functions)", (accounts) => {
  ...
  it("Updating a Zero Address as Owner by BackupOwner should not be possible.", async () => {
    await expectRevert(
      inherichain.updateOwner(constants.ZERO_ADDRESS, { from: backupOwner }),
      "Address has to be valid."
    );
  });

  it("Updating an Owner by BackupOwner should emit the ownerUpdated Event.", async () => {
